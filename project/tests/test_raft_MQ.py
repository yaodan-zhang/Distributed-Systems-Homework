...
def send_message():
    leader = get_leader()
    if leader:
        requests.put(f"{leader}/message", json={"topic": "news", "message": "Update!"}, timeout=1)
...
# Expected Output: The leader should handle multiple concurrent requests without inconsistency.

# Test: Checking committed log entries on all nodes
...
# Expected Output: All nodes should have a matching commit index after updates.
